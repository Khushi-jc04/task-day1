...
export function LoginPage() {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            ...
            {
                values.email === data.email ? values.password === data.password ? <p>{navigate("/cart")}
                    {localStorage.setItem("isLoggedin", true)
                    }</p>
                    :
                    <>
                        {
                            setErrors(prev => ({ ...prev, email: 'Invalid password' }))
                        }
            ...
            <div className="m-3 md:py-40 md:px-25">
                <form onSubmit={formik.handleSubmit}
                    ...
                    <input type={showPassword ? 'password' : 'text'} name="password" id="password"
                        ...
                        className="md:mx-4 border-b-1 border-black"
                    />
                    <span className="absolute right-118 top-95" onClick={() => {
